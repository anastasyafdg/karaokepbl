    ...
    <form method="POST" action="{{ route('login.submit') }}">
      ...
      <div class="mb-6">
        ...
        <input type="email" id="email" name="email" placeholder="nama@gmail.com"
               class="w-full px-4 py-3 rounded-lg bg-white border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400"
      ...
      <div class="mb-6">
        ...
        <div class="relative">
          <input type="password" id="password" name="password" placeholder="Masukkan Kata Sandi"
                 class="w-full px-4 py-3 rounded-lg bg-white border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 pr-10" required>
      ...
      <button type="submit"
              class="w-full bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900 transition">
        Kirim
      </button>
    </form>

    <div class="mt-6 text-center text-sm text-gray-700">
      <a href="{{ url('ganti_sandi') }}" class="hover:underline">Lupa Password?</a>
    </div>
    <div class="mt-2 text-center text-sm text-gray-700">
      Tidak punya akun?
      <a href="{{ url('registrasi') }}" class="font-bold hover:underline">Daftar Disini</a>
    </div>
  </div>
...
<script>
  ...
  togglePassword.addEventListener('click', function () {
    ...
    eyeIcon.setAttribute('stroke', type === 'text' ? '#1D4ED8' : 'currentColor');
  });
</script>
