            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100">
                     ...
                     <tbody>
                        ...
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-xs text-gray-400">{{ $pembayaran->tanggal_pembayaran->format('H:i') }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                @if($pembayaran->bukti_pembayaran)
                                    <a href="{{ asset('image/' . $pembayaran->bukti_pembayaran) }}" 
                                       target="_blank" 
                            ...
                            <td class="px-4 py-4">
                                <div class="flex gap-2 flex-col">
                                    ...
                                    <form action="{{ route('data_pembayaran.destroy', $pembayaran->id) }}" method="POST" 
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-3 py-1 text-white bg-red-600 hover:bg-red-700 rounded text-sm">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
                        ...
                        <!-- Edit Modal for each pembayaran -->
                            ...
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                        ...
                                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="editModal-{{ $pembayaran->id }}">
                                    ...
                                    <form class="p-4 md:p-5" action="{{ route('data_pembayaran.update-status', $pembayaran->id) }}" method="POST">
                                        ...
                                        <div class="grid gap-4 mb-4 grid-cols-1">
                                            <div class="col-span-1">
                                                <label for="status-{{ $pembayaran->id }}" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status Pembayaran</label>
                                                <select id="status-{{ $pembayaran->id }}" name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
